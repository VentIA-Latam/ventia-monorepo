...
class Message < ApplicationRecord
  ...
  # True when this message was generated by the AI/bot: an outgoing message with
  # no human sender that is not an echo of a reply sent from the native WhatsApp
  # app. Mirrors the frontend `getSenderRole() === "ai"` heuristic. Only these
  # messages can receive like/dislike feedback.
  def ai_generated?
    outgoing? && sender_id.nil? && !external_echo?
  end
  ...
  def webhook_data
    {
      ...
      # Feedback is per-agent and resolved in the REST serializer with the
      # requesting user's context. WS broadcasts have no current user and brand
      # new messages carry no vote, so the shape stays stable with a null here.
      feedback: nil,
